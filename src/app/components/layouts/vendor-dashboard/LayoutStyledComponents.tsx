...
const NavWrapper = styled(Box)<{compact: boolean}>({
  height: "100%",
  paddingLeft: 16,
  paddingRight: 16,
});
const NavItemButton = styled(ButtonBase)(
  ({ theme, active }: { theme?: any; active: boolean | number }) => ({
    height: 44,
    width: "100%",
    borderRadius: 8,
    marginBottom: 4,
...
const ListLabel = styled(Paragraph)(({ compact }: { compact: any }) => ({
  fontWeight: 600,
  fontSize: "12px",
  marginTop: "20px",
  marginLeft: "15px",
...
const StyledText = styled(Span)(({ compact }: { compact: any }) => ({
  whiteSpace: "nowrap",
  // paddingLeft: "0.8rem",
  transition: "all 0.15s ease",
  ...(compact && {
...
const BadgeValue = styled(Box)(
  ({ compact }: { compact: "none" | "unset" | number }) => ({
    padding: "1px 8px",
    overflow: "hidden",
    borderRadius: "300px",
...
const ChevronLeftIcon = styled(ChevronLeft)(
  ({
    ...
    compact: "none" | "unset" | number;
    sidebarcompact: "rotate(180deg)" | "rotate(0deg)" | number;
  }) => ({
...
const ChevronRightIcon = styled(ChevronRight)(
  ({ collapsed, compact, theme: { direction } }: { collapsed: number, compact: any, theme?: any }) => ({
    fontSize: 18,
    color: "white",
